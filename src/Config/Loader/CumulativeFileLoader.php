...
namespace Effiana\CacheBundle\Config\Loader;

use Effiana\CacheBundle\Config\CumulativeResource;
use Effiana\CacheBundle\Config\CumulativeResourceInfo;

/**
 * Class CumulativeFileLoader
...
abstract class CumulativeFileLoader implements CumulativeResourceLoader
{
     ...
     * @var string
     */
    protected $effianaRelativeFilePath = null;
    /** @var array  */
    ...
    /**
     * Gets relative path to a resource file in effiana subfolder
     *
     * @return string
     */
    public function getEffianaRelativeFilePath()
    {
        return $this->effianaRelativeFilePath;
    }
    /**
    ...
    public function setRelativeFilePath($relativeFilePath)
    {
        ...
        $effianaRelativeFilePath = sprintf(
            '%s/effiana%s',
        ...
        if (strpos($relativeFilePath, '/') === 0) {
            ...
            $this->effianaRelativeFilePath = $effianaRelativeFilePath;
        } else {
            $this->resource            = $relativeFilePath;
            $this->relativeFilePath    = DIRECTORY_SEPARATOR . $path;
            $this->effianaRelativeFilePath = DIRECTORY_SEPARATOR . $effianaRelativeFilePath;
        }
        $this->relativePaths = [$this->effianaRelativeFilePath, $this->relativeFilePath];
    }
    /**
     * {@inheritdoc}
     */
